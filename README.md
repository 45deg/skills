# 45deg Skills

Agent skills for 45deg workflows, packaged in the standard `skills/`
repository layout.

## Install

List the skills in this repository:

```bash
npx skills add 45deg/skills --list
```

Install a specific skill:

```bash
npx skills add 45deg/skills --skill ud-color-design
```

## Skills

### `ud-color-design`

Applies Color Universal Design and UD color principles to visual artifacts.

Use it for:

- Web UI colors, design tokens, and state styles
- Charts, maps, infographics, and slide visuals
- Print and DTP palettes that need RGB/CMYK handling
- Audits where information is communicated by color alone

## Layout

```txt
skills/
|-- README.md
|-- skills.sh.json
`-- skills/
    `-- ud-color-design/
        |-- SKILL.md
        |-- README.md
        `-- references/
```

Each skill lives under `skills/<skill-name>/` and has its own `SKILL.md`.
Supporting material such as reference notes, scripts, and assets should stay
inside the same skill folder.
